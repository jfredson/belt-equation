...
| `depends_on_any` | no | List of groups, each a list of ids; at least one id in every group must resolve before this node can. This is how a node says "by either route", for example the biology node that resolves if partial-gravity health works out or if a rotating habitat with Earth-normal gravity exists. From 2026-09-25 it is the default form whenever more than one route could satisfy a child (docs/methodology.md, "Function over route"). |
...
| `watch` | no | List of extra search terms for the weekly web scan (the Radar, docs/scan-plan.md): a project name, a mission name, a term of art, anything the criterion's plain words would miss. The scan writes its searches from the node's `name`, `resolution` and `source` and adds every term listed here. Leave it out unless the plain reading really does miss something. Checked by the export script (`scripts/export.py`) rather than the compute script, because the scan is a website-side concern; the rule is that if the field is there it holds at least one line of text. Added 2026-09-19 with the Radar. |
...
- **Links written before 2026-09-25 could name one route.** The function-over-route rule (docs/methodology.md) was first applied in the sweep of 2026-09-25 (docs/proposal-function-over-route-2026-09-25.md), which checked every link in the tree and changed seven: four now point at new function steps, and three parents had their criteria widened to the function instead. From then on, every brainstorm applies the test to each new link as it is drawn.
