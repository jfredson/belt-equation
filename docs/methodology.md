...
One more figure is reported beside the headline, from 2026-09-20: the chance of working a rotation at a station or a lunar base however far human industry has reached, which is A2's own requirement (a role whose holders rotate off Earth) met together with Tier 1 (an outpost) rather than with Tier 3. The outside review's largest structural point was that a visitor reading the question (worded differently before 2026-09-25, same meaning) may picture exactly that nearer thing, while the headline measures the narrower event of working rotations once human industry reaches beyond Earth and Mars. The headline was kept as it was, because that is what the question asks, and the nearer figure is shown beside it so the gap is visible instead of hidden. It is computed in the same play-throughs (a play-through counts when both the outpost tier and the rotation-role node held), so the two figures are always from the same run.
