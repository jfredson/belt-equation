...
10. Run the calculation. Read the results against intuition. Where the computed number disagrees badly with gut, the tree is probably missing a dependency or a node; fix the tree, not the number. FIRST RUN 2026-09-19: docs/runs/2026-09-19-first-run.md. Three results fail the gut test; four structural fixes proposed, awaiting John's ruling; step stays open until the fixes land and the run is repeated.
