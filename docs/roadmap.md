...
- Nothing from John's health record goes into this repo. What may be said is the one general paragraph in definitions.md under "The window" (a healthy forty-year-old in a demanding military career, an endurance athlete, deliberately investing in his own health trajectory), amended by John 2026-09-08 from the earlier rule of nothing at all. The access branch stays a sketch until pipeline graduation (expected May 2027).
...
- Facts stated from memory get checked against the web as the work proceeds, when a quick search would settle them, and otherwise are marked on the record as unverified (the node schema's `verify` field) so a later session picks them up. Ruled by John 2026-09-08 after a node was written as resolved on Claude's memory alone. Aggregator sites are leads, not confirmation.
...
5. Write belt-equation/scripts/compute.py against the worked example from step 2, in parallel with the brainstorm: load the tree, run it many thousands of times with random outcomes while respecting dependencies, and report the probability of each system tier and each access tier under each longevity scenario (five since 2026-09-08), plus the Contact Clause rungs separately. Keep it to plain Python with no dependencies beyond the standard library if possible. Include a check that refuses to run if any node lacks a resolution criterion. Writing it now, while the tree is still small, takes the crunch out of Phase 3 and catches schema problems early. DONE 2026-09-08: scripts/compute.py with validate, shape, run, and compare commands; it enforces all eight schema rules, refuses numbers until Phase 3, and was checked against a small test tree by hand arithmetic. Re-run `validate` after every brainstorm session.
6. Mark choice points in the access branch and list the options at each one. DONE 2026-09-08 inside the third brainstorm session: one choice group, the six-year fork, with re-enlist marked as the current plan and separation into civilian space or defence work as the alternative. Further forks are added as the branch firms up after pipeline graduation.
7. Draw the dependency edges and check for cycles. A leaf that depends on a root is a sign the horizon classes are wrong. DONE 2026-09-08: cross-factor edges drawn from the notes left at the brainstorm; two hard dependencies loosened (fusion on superconducting wire, orbital assembly on launch price); the access branch given two route nodes so the six-year fork compares different paths, plus nodes for the A0 and A1 tiers; a node added for Tier 3's site beyond the Earth-Moon system; two nodes cut (the degree, space solar sold to a grid) and two verified leaves added (fusion plant construction begun, resolved 2025; both stages of a heavy launcher recovered, open) to keep a third of the tree leaves. Every tier now has a requirement list. Validator finds no cycles.
8. Review pass: read every node aloud in plain language and cut, merge, or rename anything that fails the "someone outside the project understands it" test. DONE 2026-09-08: all sixty names passed; nine criteria carried terms an outside reader would stumble on (technosignature, galactic cosmic ray dose, kiloamp-metre, exhaust velocity and kilonewton, composite endpoint, appropriations, all-cause mortality, regolith, a bare "0.9 g") and each now gives the plain meaning first with the term once. No node cut or merged. All five verification flags cleared by search or converted to dated notes; the metformin trial's results remain unpublished in any primary source and that node stays open. The tree is stable; probabilities may begin.

## Phase 3. First probabilities and the calculation (target 2026-10-26)

9. First-pass probabilities, grouped the same way as the brainstorm sessions, with a one-line rationale each. John's estimates, with Claude proposing a number and a reason for him to accept or move. Record every number with its date. The refined window date from step 3 is needed here. Where a public forecasting site has an equivalent question, note its number beside ours as a reference, not an authority. PROGRESS: group one (window, biology) done 2026-09-08; group two (launch, energy, drive) done 2026-09-19, 16 nodes; group three (motive, regime, access, Contact Clause) done 2026-09-19, 24 nodes. All sixty nodes estimated.
10. Run the calculation. Read the results against intuition. Where the computed number disagrees badly with gut, the tree is probably missing a dependency or a node; fix the tree, not the number. FIRST RUN 2026-09-19: docs/runs/2026-09-19-first-run.md. Three results failed the gut test; four structural fixes proposed and ruled by John the same evening (regime gates reworded, child-born requirement moved to Tier 4, world draw added to the script); second run the same day. DONE 2026-09-19: headline A2 0.9 percent by 2071, 5.6 by 2095, 18 with no deadline.
11. Run the decision comparison: the whole tree once per option at the six-year service fork (re-enlist or separate, and the branches under each), using the sketch access branch, and report how the A2 number moves between options. Rough numbers are fine. This is what makes version one a decision tool rather than a forecast only, which is the reason John named as the most exciting one for building the project. FIRST PASS 2026-09-19 (docs/runs/2026-09-19-first-run.md): the fork separates by under a point in favour of separating, within the noise of a sketch access branch; re-run after pipeline graduation firms up the branch.
12. Write belt-equation/docs/methodology.md: how the numbers are computed, how the calibration score works (the standard measure of how well probabilities match outcomes, computed on resolved leaves each year), and how the choice-point comparison is run. DONE 2026-09-19; also states the case for the world-draw spread and the rule that the five scenarios are reported side by side, not combined.
27. Design and run the outside-model review protocol before anything is published (numbered 27 so the roadmap's numbers stay stable; it belongs here in Phase 3, after the first run; target 2026-11-09, before website step 24). Promoted from "Later, not now" on 2026-09-08 when John accepted the first probabilities on Claude's recommendation and asked that they be reviewed against a protocol using other frontier models. The protocol: give each factor's nodes, criteria, rationales, and numbers to at least two models other than the one that proposed them, with a fixed brief (name a missing node; challenge a criterion; argue a number up or down with a reason and a confidence level); collect the answers in a dated file under docs/reviews/; John rules on each disagreement; every change lands with a changelog line and a revision entry on the node. Done when every factor has been reviewed once and the disagreements are ruled on. PROTOCOL DRAFTED 2026-09-19 (docs/reviews/protocol.md, brief.md, scripts/review_packets.py, first packets), ruled in force by John the same day; sessions in progress, responses under docs/reviews/.
